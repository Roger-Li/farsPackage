...
#'
#' @details The data type in the columns is automatically parsed by the
...
#'   an error.
#'
#' @param filename path to the input csv data file, as a character string.
#'
#' @return A tibble data frame, enhanced implementation of data.frame
...
#' \donttest{data2015 <- fars_read('fars_data/accident_2015.csv.bz2')}
#'
#' @import readr
#' @import dplyr
#'
...
#' Creates a filename adding year information
#'
#' @description Inserts a year number into a base string and builds a new
#'   filename.
#'
...
#' Reads a vector of files and returns the months and years
#'
#' The function reads a list of csv data files, specified by year, and returns
#'   a list of tibbles \code{\link{tibble}} with two columns: month and year.
#'
...
#' Reads a vector of files and returns the # of observation by month and year
#'
#' @description The function takes a vector of year numbers, reads the corresponding files
#'   (using \code{\link{fars_read_years}}) and counts the number of rows in aggregated 
#'   by month number.
#'   Then it builds a table with one row for each month, and one column
#'   for each file / year, and the values represent the total number of
#'   observations.
#'
#' @note
#' The data files must be in the current working directory (see also
...
#' @param years a vector of years, as integers.
#'
#' @return A tibble with 12 rows and 1 column for each year.
#'
